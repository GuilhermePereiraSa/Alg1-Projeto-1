...
void analise_tempo_execucao(void (*func)(LISTA*, int, int, int, CAMINHO*), LISTA* lista_input, int numCidades, int cidadeOrigem, int numArestas, CAMINHO* caminho_menor_custo) {
...
int main(void){
    ...
    // Lendo o número de cidades, cidade de origem, e o número de arestas em linhas separadas
    scanf("%d", &numCidades);   // Primeira linha: número de cidades
    scanf("%d", &cidadeOrigem); // Segunda linha: cidade de origem
    scanf("%d", &numArestas);   // Terceira linha: número de arestas
    ...
    // Lendo as arestas, uma por linha
    for(int i = 1; i <= numArestas; i++){
        scanf("%d", &tmp_cidade_A); // Cidade A
        scanf("%d", &tmp_cidade_B); // Cidade B
        scanf("%d", &tmp_distancia); // Distância
        lista_inserir(lista_input, aresta_criar(tmp_cidade_A, tmp_cidade_B, tmp_distancia, i));
    }

    // Chamada à função de análise de tempo de execução
    analise_tempo_execucao(busca_menor_custo, lista_input, numCidades, cidadeOrigem, numArestas, caminho_menor_custo);
