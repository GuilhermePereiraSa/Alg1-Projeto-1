...
void explorarRotas(Aresta[], int, int, PILHA *, bool[], int, int, int *,
                   PILHA *);
void permutacaoIterativa(Aresta[], int, PILHA *, int *, int);

// Função wrapper para adaptar a assinatura da função `explorarRotas`
void wrapperExplorarRotas(Aresta arestas[], int numArestas, PILHA *melhorRota,
                          int *menorCusto, int numCidades) {
  ...
  explorarRotas(arestas, numArestas, numCidades, pilha, visitado, cidadeOrigem,
                0, menorCusto, melhorRota);

  pilha_apagar(&pilha);
}

// Função para medir o tempo de execução de uma função
void medirTempoExecucao(void (*funcao)(Aresta[], int, PILHA *, int *, int),
                        Aresta arestas[], int numArestas, PILHA *melhorRota,
                        int *menorCusto, int numCidades) {
  clock_t inicio, fim;
  double tempoGasto;
  ...
  tempoGasto =
      ((double)(fim - inicio)) /
      CLOCKS_PER_SEC; // Calcula o tempo gasto em segundos
                      // Cria-se um período dividindo pelo clocks_per_sec.
  printf("Tempo de execução: %f segundos\n", tempoGasto);
}

// Função para encontrar a distância entre duas cidades
int encontrarDistancia(Aresta arestas[], int numArestas, int cidadeA,
                       int cidadeB) {
...
int calcularCusto(Aresta arestas[], int numArestas, PILHA *pilha) {
  ...
  int distanciaRetorno =
      encontrarDistancia(arestas, numArestas, item_get_chave(ultimaCidade),
                         item_get_chave(cidadeOrigem));
...
void explorarRotas(Aresta arestas[], int numArestas, int numCidades,
                   PILHA *pilha, bool visitado[], int cidadeAtual,
                   int custoAtual, int *menorCusto, PILHA *melhorRota) {
...
// verifica se a rota está completa, não contando com a de origem
  if (pilha_tamanho(pilha) == numCidades - 1) {
    ...
    int distanciaRetorno = encontrarDistancia(arestas, numArestas, cidadeTopo,
                                              item_get_chave(pilha->item[0]));
  ...
  for (int i = 1; i <= numCidades; i++) {
    if (!visitado[i]) {
      ...
      if (distancia != INT_MAX && custoAtual + distancia < *menorCusto) {
        ...
        explorarRotas(arestas, numArestas, numCidades, pilha, visitado, i,
                      custoAtual + distancia, menorCusto, melhorRota);
...
void permutacaoIterativa(Aresta arestas[], int numArestas, PILHA *melhorRota,
                         int *menorCusto, int numCidades) {
  PILHA *pilha = pilha_criar();
  bool visitado[numCidades];
  for (int i = 0; i < numCidades; i++) {
...
int main() {
  ...
  for (int i = 0; i < numArestas; i++) {
    fscanf(arquivo, "%d %d %d", &arestas[i].cidadeA, &arestas[i].cidadeB,
           &arestas[i].distancia);
  }
  ...
  printf("Backtracking:\n");
  medirTempoExecucao(wrapperExplorarRotas, arestas, numArestas, melhorRota,
                     &menorCusto, numCidades);

  // Medir tempo de execução da permutação iterativa
  printf("Permutação iterativa:\n");
  medirTempoExecucao(permutacaoIterativa, arestas, numArestas, melhorRota,
                     &menorCusto, numCidades);
